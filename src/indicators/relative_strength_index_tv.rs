...
#[derive(Debug, Clone)]
pub struct RelativeStrengthIndexTv {
    period: usize,
    up_rma_indicator: Rma,
    down_rma_indicator: Rma,
    prev_val: f64,
    is_new: bool,
}

impl RelativeStrengthIndexTv {
    pub fn new(period: usize) -> Result<Self> {
        Ok(Self {
            period,
            up_rma_indicator: Rma::new(period)?,
            down_rma_indicator: Rma::new(period)?,
            prev_val: 0.0,
            is_new: true,
        })
    }
...
impl Period for RelativeStrengthIndexTv {
    fn period(&self) -> usize {
        self.period
    }
}

impl Next<f64> for RelativeStrengthIndexTv {
    type Output = f64;
    ...
    fn next(&mut self, input: f64) -> Self::Output {
        ...
        self.prev_val = input;
        let up_rma = self.up_rma_indicator.next(up);
        let down_rma = self.down_rma_indicator.next(down);
        100.0 * up_rma / (up_rma + down_rma)
    }
}

impl<T: Close> Next<&T> for RelativeStrengthIndexTv {
    type Output = f64;
...
impl Reset for RelativeStrengthIndexTv {
    fn reset(&mut self) {
        self.is_new = true;
        self.prev_val = 0.0;
        self.up_rma_indicator.reset();
        self.down_rma_indicator.reset();
    }
}

impl Default for RelativeStrengthIndexTv {
    fn default() -> Self {
        Self::new(14).unwrap()
    }
}

impl fmt::Display for RelativeStrengthIndexTv {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "RSI_TV({})", self.period)
    }
}
